...
class Webcam:
    ...
    def get_freetype2_font(self):
        ...
        font = ImageFont.truetype('E:\\Desktop\\qbr-master\\src\\assets\\arial-unicode-ms.ttf', 30)
        return font
    ...
    def get_text_size(self, text, size=TEXT_SIZE):
        ...
        font = self.get_freetype2_font()
        return ImageFont.getsize(text, font)
    ...
    def draw_current_color_to_calibrate(self, frame):
        ...
        if self.done_calibrating:
            ...
            for index, text in enumerate(messages):
                font_size
                y = y_offset + (30 + 10) * index
                cv2.putText(frame, 
                text, 
                (int(self.width / 2 - 200 / 2), y), 
                font, 0.8, 
                (0, 255, 255), 
                2, 
                cv2.LINE_4)
            ...
            cv2.putText(frame, 
                text, 
                (int(self.width / 2 - 200 / 2), y_offset), 
                font, 0.8, 
                (0, 255, 255), 
                2, 
                cv2.LINE_4)
    ...
    def draw_calibrated_colors(self, frame):
        ...
        for index, (color_name, color_bgr) in enumerate(self.calibrated_colors.items()):
            ...
            font = cv2.FONT_HERSHEY_SIMPLEX
            cv2.putText(frame,  i18n.t(color_name), (20, y1 + 3), font, 0.8, (0, 255, 255), 2, cv2.LINE_4)
